  ...
  ) //
  {
    ...
    for (; processed_buckets < num_non_empty_buckets; ++processed_buckets, ++buckets) {
      ...
      for (uint64_t pilot = 0; true; ++pilot) {
        uint64_t hashed_pilot = PTHASH_LIKELY(pilot < search_cache_size) ? hashed_pilots_cache[pilot] : mix(pilot);
    ...
    if (config.num_threads > 1) {
      if (config.num_threads > std::thread::hardware_concurrency()) {
        throw std::invalid_argument(
          "parallel search should use at most " + std::to_string(std::thread::hardware_concurrency()) + " threads"
        );
      }
      search_parallel(
        num_keys,
