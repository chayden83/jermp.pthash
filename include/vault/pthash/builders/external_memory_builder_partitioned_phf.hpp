...
namespace pthash {
  ...
  struct external_memory_builder_partitioned_phf {
    ...
    ) //
    {
      ...
      m_builders.init(
        config.tmp_dir, static_cast<uint64_t>(clock_type::now().time_since_epoch().count()), num_partitions
      );
      ...
      if (failure) {
        ...
        throw std::runtime_error("each partition must contain at least one key: use less partitions");
      }
      ...
      if (config.num_threads > 1) { // parallel
        ...
        for (; i != num_partitions; ++i) {
          ...
          uint64_t partition_bytes =
            internal_memory_builder_single_phf<hasher_type, Bucketer>::estimate_num_bytes_for_construction(
              size, partition_config
            );
          ...
          in.read(reinterpret_cast<char*>(p.data()), static_cast<std::streamsize>(size * sizeof(hash_type)));
          in.close();
          std::remove(partitions[i].filename().c_str());
        ...
        for (uint64_t i = 0; i != num_partitions; ++i) {
          mm::file_source<hash_type> partition(partitions[i].filename(), mm::advice::sequential);
  ...
  private:
    ...
    private:
      std::string get_partition_filename(uint64_t partition) const {
        ...
        filename << m_dir_name << "/pthash.tmp.run" << m_run_identifier << ".partition" << partition << ".bin";
        return filename.str();
      }
  ...
  public:
    builders_files_manager<internal_memory_builder_single_phf<hasher_type, Bucketer>> const& builders() const {
      return m_builders;
    }
  ...
  private:
    ...
    struct meta_partition {
      ...
      void flush() {
        ...
        out.write(reinterpret_cast<char const*>(m_hashes.data()), m_hashes.size() * sizeof(hash_type));
        out.close();
        m_hashes.clear();
      }
