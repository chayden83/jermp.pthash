...
namespace pthash {
  ...
  struct external_memory_builder_single_phf {
    ...
    // non copyable
    external_memory_builder_single_phf& operator=(external_memory_builder_single_phf const&) = delete;
    ...
    ) //
    {
      ...
      if (config.verbose) {
        ...
        uint64_t           peak =
          num_keys * (sizeof(bucket_payload_pair) + sizeof(uint64_t)) + (num_keys + num_buckets) * sizeof(uint64_t);
      }
      ...
      try {
        ...
        { // search
          ...
          auto     pilots         = tfm.get_multifile_pairs_writer(num_non_empty_buckets, ram_for_pilots, 1, 0);
        ...
        if (config.minimal and num_keys < table_size) { // fill free slots
          ...
          buffered_file_t<uint64_t> writer(tfm.get_free_slots_filename(), ram - bitmap_taken_bytes);
  ...
  private:
    ...
    template <typename T>
    struct buffer_t {
      buffer_t(uint64_t ram)
        : m_buffer_capacity(ram / sizeof(T)) {
    ...
    struct buckets_t { // merger
      ...
      buckets_t(std::vector<std::string> const& filenames, uint64_t ram, std::vector<bool>& used_bucket_sizes)
    ...
    private:
      void ensure_capacity(uint64_t bucket_size) {
        if (bucket_size + 1 > m_buffer_capacity) {
          std::sort(m_non_empty_buckets.begin(), m_non_empty_buckets.end(), [&](uint64_t i, uint64_t j) {
      ...
      void flush_i(uint64_t i) {
        ...
        m_outs[i].write(reinterpret_cast<char const*>(m_buffers[i].data()), m_buffers[i].size() * sizeof(uint64_t));
    ...
    struct buckets_iterator_t {
      buckets_iterator_t(std::vector<std::pair<bucket_size_type, std::string>> const& sizes_filenames)
        : m_sizes(sizes_filenames.size())
        , m_sources(sizes_filenames.size()) {
    ...
    protected:
      void flush_impl(std::vector<bucket_payload_pair>& buffer) {
        ...
        if (m_num_threads_sort > 1) { // parallel
          ...
          std::ofstream out(m_filenames[m_num_pairs_files], std::ofstream::out | std::ofstream::binary);
          if (!out.is_open()) {
            throw std::runtime_error("cannot open temporary file (write)");
          }
          ++m_num_pairs_files;
          std::sort(buffer.begin(), buffer.end());
          out.write(reinterpret_cast<char const*>(buffer.data()), size * sizeof(bucket_payload_pair));
          out.close();
        }
      }
    ...
    private:
      ...
      static uint64_t get_balanced_ram(uint64_t num_pairs, uint64_t ram) {
        ...
        uint64_t num_temporary_files                   = (num_pairs + num_pairs_per_file - 1) / num_pairs_per_file;
        uint64_t balanced_num_pairs_per_temporary_file = (num_pairs + num_temporary_files - 1) / num_temporary_files;
        uint64_t balanced_ram = balanced_num_pairs_per_temporary_file * sizeof(bucket_payload_pair);
    ...
    struct temporary_files_manager {
      ...
      multifile_pairs_writer get_multifile_pairs_writer(
        ...
        return multifile_pairs_writer(
          filenames, m_num_pairs_files, num_pairs, ram, num_threads_sort, ram_parallel_merge
        );
      }
    ...
    private:
      std::string get_pairs_filename(uint32_t file_id) const {
        std::stringstream filename;
        filename << m_dir_name << "/pthash.tmp.run" << m_run_identifier << ".pairs" << file_id << ".bin";
        return filename.str();
      }

      std::string get_buckets_filename(bucket_size_type bucket_size) const {
        std::stringstream filename;
        filename << m_dir_name << "/pthash.tmp.run" << m_run_identifier << ".size" << static_cast<uint32_t>(bucket_size)
                 << ".bin";
        return filename.str();
      }
    ...
    void map(
      ...
      auto writer = tfm.get_multifile_pairs_writer(num_keys, ram - ram_parallel_merge, num_threads, ram_parallel_merge);
      try {
